...
class wpl_sef
{
	...
	public static function set_location_vars($parameters)
	{
		...
		foreach($location_fields as $column=>$location_field)
		{
			...
			$query = "SELECT `id` FROM `#__wpl_location".$location_id."` WHERE `name`='".urldecode($rendered_parameters[$location_field])."'";
    ...
    {
        $main_permalink = apply_filters('wpl_change_permalink', wpl_global::get_setting('main_permalink'));
