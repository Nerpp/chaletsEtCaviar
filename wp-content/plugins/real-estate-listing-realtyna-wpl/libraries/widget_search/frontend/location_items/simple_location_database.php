...
if($show == 'simple_location_database' and !$done_this)
{
	...
	if($current_values['zip_id'])
	{
		...
		if(count($locations))
		{
			...
			$html .= '<option value="-1">'.__((trim($location_settings['locationzips_keyword']) != '' ? $location_settings['locationzips_keyword'] : 'Select'), 'real-estate-listing-realtyna-wpl').'</option>';
