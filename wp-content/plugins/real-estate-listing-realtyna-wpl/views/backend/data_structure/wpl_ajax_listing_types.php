...
class wpl_data_structure_controller extends wpl_controller
{
	...
	private function gicon_upload_file()
	{
		...
		else
		{
			...
			{
				$error .= __("Icons should not be bigger than 500KB!", 'real-estate-listing-realtyna-wpl');
				@unlink($file);
			}
	...
	private function remove_listing_type($listing_type_id, $confirmed = 0)
	{
		...
		$res = (int) $res;
		$message = $res ? __('Listing type removed from WPL successfully.', 'real-estate-listing-realtyna-wpl') : __('Error Occured.', 'real-estate-listing-realtyna-wpl');
		$data = NULL;
