...
<table class="widefat page">
    <thead>
        <tr>
        	<th scope="col" class="size-1 manage-column" colspan="2"><?php echo __('Listing Types', 'real-estate-listing-realtyna-wpl'); ?></th>
            <th colspan="5">
    ...
    <tbody class="sortable_list_type">
        <?php foreach($this->listing_types as $id=> $wp_listing_type): ?>
            <tr id="item_row_<?php echo $wp_listing_type['id']; ?>">
                ...
                <td><?php echo __($wp_listing_type['name'], 'real-estate-listing-realtyna-wpl'); ?></td>
                <td class="manager-wp">
                ...
                <td class="manager-wp">
                    ...
                    <a data-realtyna-lightbox data-realtyna-lightbox-opts="reloadPage:true" href="#wpl_data_structure_edit_div" class="action-btn icon-edit" onclick="wpl_generate_edit_page_listing_type(<?php echo $wp_listing_type['id']; ?>);"></a>
                    <?php endif; ?>
                </td>
                <td class="manager-wp">
...
<table class="widefat page widefat-margint10" cellspacing="0" width="100%">
    <thead>
        <tr>
            <th>
                <?php echo __('Google Icon', 'real-estate-listing-realtyna-wpl'); ?>
            </th>
        </tr>
    ...
    <tbody>
        <tr>
            <td>
                <?php
                $params = array('html_element_id'=>'wpl_gicon_file', 'html_path_message'=>'.wpl_data_structure_list_gicon .wpl_show_message', 'html_ajax_loader'=>'#wpl_gicon_ajax_loader', 'request_str'=>'admin.php?wpl_format=b:data_structure:ajax_listing_types&wpl_function=gicon_upload_file&_wpnonce='.$this->nonce);
        ...
        <tr>
            <td class="gmarker-icon-wp" id="wpl_gicon_listing">
                <?php foreach($this->listing_gicons as $index=>$listing_gicon): if(strpos($listing_gicon, 'multiple') !== false) continue; ?>
                    <div class="gmarker-icon" id="gicon<?php echo $index ?>">
                        ...
                        <span class="ajax-inline-save" id="wpl_gicon_ajax_loader_<?php echo __($index, 'real-estate-listing-realtyna-wpl'); ?>"></span>
                    </div>
                <?php endforeach; ?>
            </td>
        </tr>
        <tr>
            <td class="gmarker-icon-wp" id="wpl_gicon_listing_multiple">
                <?php foreach($this->listing_gicons as $index=>$listing_gicon): if(strpos($listing_gicon, 'multiple') === false) continue; ?>
                    <div class="gmarker-icon" id="multiple_gicon<?php echo $index ?>">
                        <img src="<?php echo wpl_global::get_wpl_asset_url('img/listing_types/gicon/'.$listing_gicon); ?>" alt="" />
                        <?php if(wpl_users::is_super_admin()): ?><span class="action-btn icon-recycle-3" onclick="wpl_gicon_delete('<?php echo $listing_gicon; ?>', 0, '<?php echo $index; ?>');"></span><?php endif; ?>
                        <span class="ajax-inline-save" id="wpl_gicon_ajax_loader_<?php echo __($index, 'real-estate-listing-realtyna-wpl'); ?>"></span>
                    </div>
...
<table class="widefat page widefat-margint10" cellspacing="0" width="100%">
    <thead>
        <tr>
            <th>
                <?php echo __('Multiple Marker Icon', 'real-estate-listing-realtyna-wpl'); ?>
            </th>
        </tr>
    </thead>
    <tbody>
        <tr>
            <td>
                <p><?php echo __('It will show when there are multiple listings at the same geo position.', 'real-estate-listing-realtyna-wpl'); ?></p>
            </td>
        </tr>
        <tr>
